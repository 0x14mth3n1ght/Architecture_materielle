...
Char *s_;
{
  ...
  do {
	...
	while (k != NULL) {
	  ...
	  WITH1->xx2 = k->xx2;
	  WITH1->yy2 = k->yy2;
          writedesc((char *) &GETFBUF(V.f, filerec).b[16*(V.p & 15)],WITH1);
	  V.p++;
	  k = k->next;
	}
	while (V.p < 16) {
...
/* p2c: loged.text, line 1701: Note: OPEN does not specify a name [181] */
	...
	goto _Ltry7;
      ...
      SETUPBUF(V.f, filerec);
      GET(V.f, filerec);
      ng = getshortsw((char *) &GETFBUF(V.f, filerec).b[4]);
...
/* p2c: loged.text, line 1723: Note: Character >= 128 encountered [281] */
	...
	if (li[i] != NULL) {
	  WITH = readdesc((char *) &GETFBUF(V.f, filerec).b[16*((i) & 15)]);
	  if (0 /* WITH->numpins > maxpins */) { /* can never happen */
 ...
 * Note: Modification of string length may translate incorrectly [146] */
	  ...
	  if (!k->flag.U3.expanded) {
	    ...
	    for (i = 0; i < FORLIM; i++) {
	      readint(&V);
              pn = readpnum((char *) &GETFBUF(V.f, filerec).b[4*V.p]);
              k->pinoff[i].x = pn.x;
              k->pinoff[i].y = pn.y;
              k->pinnum[i] = pn.num;
	      V.p++;
...
Static Void dumpcmd()
{
  ...
  if (curkind == NULL)
    ...
    for (i = 0; i < FORLIM; i++) {
      WITH = &kind.vector[i];
      if (WITH->col != curcol) {
	curcol = WITH->col;   /* parenthesis below is a guess */
...
Local Void docmd()
{
  ...
  strcpy_overlap(argument, argument + i);
